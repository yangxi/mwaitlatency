...
static char *usage = "test_latency 0x40f02f000 1 UOPS_RETIRED:u:k UOPS_RETIRED:u:k:t INSTRUCTION_RETIRED:u:k INSTRUCTION_RETIRED:u:k:t";
int main(int ac, char **av)
{
  ...
  //report the header
  printf("#id:SLEEP_LATENCY(cycles) WAKEUP_LATENCY [MWAIT_LATENCY] [MWAIT_WAKEUP]\n");
  ...
  for (i=0; i<100; i=i+1) {
    ...
    m_tsc[2] = rdtsc();
    int end_index = report_stat(i, m_tsc, worker_tsc, k_tsc, logs);
    unsigned long sleep_latency = logs[end_index] - m_tsc[0];
    unsigned long wakeup_latency = m_tsc[2] - logs_up[0];
    if (sleep_style == NANONAP){
      unsigned long mwait_sleep = logs[end_index] - k_tsc[1];
      unsigned long mwait_wakeup = k_tsc[2] - logs_up[0];
      printf("%d:%ld %ld %ld %ld\n", i , sleep_latency, wakeup_latency, mwait_sleep, mwait_wakeup);
    }else {
...
unsigned long report_stat(int iter, unsigned long *m_tsc, unsigned long *w_tsc, unsigned long *k_tsc, unsigned long *logs)
{
  ...
  unsigned long mwait_latency = logs[end_index] - k_tsc[1];
  //fprintf(stderr,"%ld\n", mwait_latency);
